class ItemsController < ApplicationController
    def index
      if params[:category_id]
        ...
        render json: {items: Item.includes(:categories).where(categories: {id: ids})}
      else
    ...
    def create
      new_item = Item.new("name": item_params["name"], "description": item_params["description"],"image_url": item_params["description"], "address": item_params["address"]  )
        if new_item.save
          item = Item.last
